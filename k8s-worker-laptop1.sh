#!/usr/bin/env bash
#
# k8s-worker-laptop1.sh
# -----------------------------------------------------------------------------
# Installs containerd + Kubernetes on "laptop1" (192.168.0.101),
...
# ------------------------------------------------------------------------------
echo "[Worker1] Step 1: Enabling IPv4 forwarding..."
...
# Step 2: Disable swap
# ------------------------------------------------------------------------------
echo "[Worker1] Step 2: Disabling swap..."
sudo swapoff -a
...
# ------------------------------------------------------------------------------
echo "[Worker1] Step 3: Installing containerd..."
sudo apt-get update
sudo apt-get install -y containerd

echo "[Worker1] Configuring containerd..."
...
# ------------------------------------------------------------------------------
echo "[Worker1] Step 4: Installing kubeadm, kubelet, kubectl..."
...
# ------------------------------------------------------------------------------
echo "[Worker1] Step 5: Retrieving /tmp/kubeadm_join_cmd.sh from master..."
scp -o StrictHostKeyChecking=no "${MASTER_HOST}:/tmp/kubeadm_join_cmd.sh" /tmp/

echo "[Worker1] Running join command..."
sudo bash /tmp/kubeadm_join_cmd.sh

echo "---------------------------------------------------"
echo "[Worker1] Worker joined the cluster successfully!"
echo "---------------------------------------------------"
